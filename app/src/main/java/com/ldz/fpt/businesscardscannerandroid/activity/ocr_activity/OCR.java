...
import com.ldz.fpt.businesscardscannerandroid.activity.LifeCycleAdapter;
import com.ldz.fpt.businesscardscannerandroid.activity.MonitorActivity;
import com.ldz.fpt.businesscardscannerandroid.activity.crop_image_activity.CropImageScaler;
import com.ldz.fpt.businesscardscannerandroid.utils.SavePixTask;
import com.ldz.fpt.businesscardscannerandroid.utils.StringUtil;
import com.ldz.fpt.businesscardscannerandroid.utils.Util;

import java.io.File;

public class OCR extends LifeCycleAdapter implements OcrProgressListener {
    ...
    public static final String ORIGINAL_PIX_NAME = "last_scan";
    ...
    public OCR(final MonitorActivity activity, final Messenger messenger) {
    ...
    @Override
    public synchronized void onActivityDestroyed(MonitorActivity activity) {
        mIsActivityAttached = false;
        cancel();
    }

    @Override
    public synchronized void onActivityResumed(MonitorActivity activity) {
        mIsActivityAttached = true;
    }
            ...
            public void run() {
                try {
                    ...
                    for (int i = 0; i < columnCount; i++) {
                        ...
                        htmlText.append(mTess.getUTF8Text());
     ...
     */
    public void startOCRForSimpleLayout(final Context context, final String lang, final Pix pixs, int width, int height, final int[] listPoints) {
        if (pixs == null) {
            throw new IllegalArgumentException("Source pix must be non-null");
        }
        mPreviewHeightUnScaled = height;
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    for (int i = 0; i < listPoints.length; i += 4) {
                        mTess.setRectangle(listPoints[i], listPoints[i + 1], listPoints[i + 2], listPoints[i + 3]);
                        synchronized (OCR.this) {
                            if (mStopped) {
                                return;
                            }
                        ...
                        htmlText += mTess.getHtmlText();
                    }
                    htmlText = "<raw_output>" + htmlText + "</raw_output>";
                    htmlText = StringUtil.unAccent(htmlText);
                    synchronized (OCR.this) {
                        if (mStopped) {
                            return;
                        }
