...
# Sets the named variable to given languageCode. Aborts if the languageCode is missing or
# if it looks like a flag.
# Usage: parse_value VAR_NAME VALUE
parse_value() {
    local val="$2"
    if [[ -z $val ]]; then
        err_exit "Missing languageCode for variable $1"
        exit
    fi
    if [[ ${val:0:2} == "--" ]]; then
        err_exit "Invalid languageCode $val passed for variable $1"
        exit
    fi
    eval $1=\"$val\"
}
